...
class StripeProcessor:
    def __init__(self, card: str = "", month: str = "01", year: str = str(datetime.now().year + 2), cvv: str = "000"):
        ...
        self.cvv = cvv
        self.zip = "10080"
        # Get SK from environment or use provided
        self.sk = os.getenv("STRIPE_SECRET_KEY", "")
    ...
    try:
        ...
        try:
            async with aiohttp.ClientSession() as session:
                result = loop.run_until_complete(processor.Auth(session))
                bin_info = loop.run_until_complete(processor.CheckBin(session))
                ...
                if result.get('status') == 'APPROVED':
                    result['message'] = '✅ Card Approved - Auth Successful'
                elif result.get('status') == 'OTP_REQUIRED':
                    result['message'] = '🔐 OTP Required - 3D Secure Authentication Needed'
                elif result.get('status') == 'DECLINED':
                    result['message'] = '❌ Card Declined - Please try another card'
                elif result.get('status') == 'DECLINED_INSUFFICIENT_FUNDS':
                    result['message'] = '❌ Insufficient Funds - Card has insufficient balance'
                elif result.get('status') == 'DECLINED_EXPIRED':
                    result['message'] = '❌ Card Expired - Card has expired'
                elif result.get('status') == 'DECLINED_INVALID':
                    result['message'] = '❌ Invalid Card Details - Check card information'
                else:
                    result['message'] = f"⚠️ {result.get('response', 'Unknown status')}"
...
def stripe_charge_get():
    ...
    try:
        ...
        try:
            async with aiohttp.ClientSession() as session:
                result = loop.run_until_complete(processor.Charge(session, amount, currency, description))
                bin_info = loop.run_until_complete(processor.CheckBin(session))
                result['bin_info'] = bin_info
                result['card'] = cc_string
                result['card_masked'] = f"{cc[:6]}******{cc[-4:]}"
                ...
                if result.get('status') == 'CHARGED':
                    result['message'] = f'✅ Charged ${amount/100:.2f} - Payment Successful'
                elif result.get('status') == 'AUTHORIZED':
                    result['message'] = f'✅ Authorized ${amount/100:.2f} - Pending Capture'
                elif result.get('status') == 'OTP_REQUIRED':
                    result['message'] = '🔐 OTP Required - 3D Secure Authentication Needed'
                elif result.get('status') == 'DECLINED':
                    result['message'] = '❌ Card Declined - Please try another card'
                    ...
                    result['message'] = '❌ Invalid Card Details - Check card information'
                elif result.get('status') == 'DECLINED_DO_NOT_HONOR':
                    result['message'] = '❌ Do Not Honor - Issuer declined the transaction'
                elif result.get('status') == 'DECLINED_PICKUP_CARD':
                    result['message'] = '⚠️ Pickup Card - Issuer requested card pickup'
                elif result.get('status') == 'DECLINED_LOST_STOLEN':
                    result['message'] = '⚠️ Lost/Stolen Card - Card reported lost or stolen'
                else:
                    result['message'] = f"⚠️ {result.get('response', 'Unknown status')}"
...
def stripe_auth_and_charge_get():
    ...
    try:
        ...
        try:
            async with aiohttp.ClientSession() as session:
                result = loop.run_until_complete(processor.AuthAndCharge(session, amount, currency, description))
                bin_info = loop.run_until_complete(processor.CheckBin(session))
                ...
                if result.get('auth'):
                    ...
                    # Add status message for auth
                    if result['auth'].get('status') == 'APPROVED':
                        result['auth']['message'] = '✅ Auth Approved'
                    elif result['auth'].get('status') == 'OTP_REQUIRED':
                        result['auth']['message'] = '🔐 Auth - OTP Required'
                    else:
                        result['auth']['message'] = f"❌ Auth {result['auth'].get('status', 'Declined')}"
                ...
                if result.get('charge'):
                    ...
                    # Add status message for charge
                    if result['charge'].get('status') == 'CHARGED':
                        result['charge']['message'] = f'✅ Charged ${amount/100:.2f} - Payment Successful'
                    elif result['charge'].get('status') == 'OTP_REQUIRED':
                        result['charge']['message'] = '🔐 Charge - OTP Required'
                    else:
                        result['charge']['message'] = f"❌ Charge {result['charge'].get('status', 'Declined')}"
